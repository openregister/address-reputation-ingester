...
class CollectionNameTest extends FunSuite {
  ...
  test("format CollectionName") {
    assert(CollectionName("fooey", 1, Some(2)).toString === "fooey_1_002")
    assert(CollectionName("fooey", 40, Some(13)).toString === "fooey_40_013")
  }

  test("CollectionName prefix") {
    assert(CollectionName("fooey", 1, Some(2)).toPrefix === "fooey_1")
    assert(CollectionName("fooey", 40, None).toPrefix === "fooey_40")
  }

  test("parse via apply") {
    assert(CollectionName("") === None)
    assert(CollectionName("foo") === None)
    assert(CollectionName("foo_bar") === None)
    assert(CollectionName("foo_bar_baz") === None)
    assert(CollectionName("abp_40_002") === Some(CollectionName("abp", 40, Some(2))))
